...
test("sync --dry-run reports changes without writing files or state", (t) => {
  ...
  assert.match(result.output, /Managed file preview complete \(1 created, 0 updated, 0 preserved, 60 baselines adopted, 1 unchanged\)\./);
...
test("sync summary distinguishes unchanged files from preserved edits", (t) => {
  ...
  assertSuccess(result);
  assert.match(result.output, /Managed file refresh complete \(0 created, 0 updated, 0 preserved, 0 baselines adopted, 61 unchanged\)\./);
});
