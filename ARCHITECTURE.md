...
- A Claude Code installable plugin lives at `src/adapter-assets/claude-code/plugin/` (source) and `neuroplast/adapter-assets/claude-code/plugin/` (installed). It packages the bootstrap skill, routing skill, execute-act skill, orchestrator agent, planner agent, and a `UserPromptSubmit` enforcement hook (`hooks/hooks.json` → `hooks/neuroplast-gate.js`) so Claude Code users can install Neuroplast workflow support without manual file copying. The gate hook injects the mandatory startup sequence and canonical short-prompt routing into context on every prompt, making the contract harness-enforced rather than relying on advisory `CLAUDE.md` text alone (which the model can skip on tasks that look trivial). Plugin files and the corresponding adapter assets must be kept in sync on release.
