...
## Recommended Prompt
`Use the Neuroplast project mind contract as the source of truth. Read neuroplast/WORKFLOW_CONTRACT.md, neuroplast/manifest.yaml, neuroplast/capabilities.yaml, any active workflow extensions declared in the manifest, and the current instruction file. Load current project state from files, keep all changes within the documented file contract, avoid destructive overwrites, and complete plan, changelog, and learning updates.`
