...
## Recommended Prompt
`Work within the Neuroplast project mind contract. Read neuroplast/WORKFLOW_CONTRACT.md, neuroplast/manifest.yaml, neuroplast/capabilities.yaml, any active workflow extensions declared in the manifest, and the current instruction file. If the repository defines shared interaction-routing rules, use them before interpreting short prompts. Load current project state from files, keep updates non-destructive, preserve required artifact paths, and finish plan, changelog, and learning steps.`
